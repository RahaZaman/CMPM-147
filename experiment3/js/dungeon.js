...
// p2_solution.js file code from Glitch.com
...
function generateGrid(numCols, numRows) {
  ...
  for (let k = 0; k < 5; k++) {
    ...
    let roomTopLeftX = floor(random(1, numCols - 5)); // Randomly choose the top-left corner X-coordinate
    let roomTopLeftY = floor(random(1, numRows - 5)); // Randomly choose the top-left corner Y-coordinate
    let roomBottomRightX = roomTopLeftX + floor(random(4, 8)); // Calculate the bottom-right corner X-coordinate
    let roomBottomRightY = roomTopLeftY + floor(random(4, 8)); // Calculate the bottom-right corner Y-coordinate
  ...
  for (let room of rooms) {
    let chestX = floor(random(room.topLeftX + 1, room.bottomRightX - 1)); // Randomly choose X-coordinate inside the room
    let chestY = floor(random(room.topLeftY + 1, room.bottomRightY - 1)); // Randomly choose Y-coordinate inside the room
...
function connectRooms(grid, room1, room2) {
  let corridorStartX = floor(random(room1.topLeftX, room1.bottomRightX));
  let corridorStartY = floor(random(room1.topLeftY, room1.bottomRightY));
  let corridorEndX = floor(random(room2.topLeftX, room2.bottomRightX));
  let corridorEndY = floor(random(room2.topLeftY, room2.bottomRightY));
...
function drawGrid(grid) {
  ...
  animationOffset = millis() * 0.2; // Adjust speed as needed
