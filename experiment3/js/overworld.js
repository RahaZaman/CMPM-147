...
// Define sketchOne function from p2_base.js
...
// Define sketchTwo function from p2_solution.js
var sketchTwo = function(p) {
  function generateGrid(numCols, numRows) {
    // Implementation from p2_solution.js
    let grid = [];
  ...
  // Other functions from p2_solution.js
  ...
  function makeCloud(cloudx, cloudy) {
    ...
    ellipse(cloudx + 10, cloudy + 10, 50, 30);
    ellipse(cloudx - 20, cloudy + 10, 50, 30);
  }
  ...
  function drawGrid(grid) {
    ...
    for (let i = 0; i < grid.length; i++) {
      for (let j = 0; j < grid[i].length; j++) {
        ...
        if (gridCheck(grid, i, j, "_")) {
          placeTile(i, j, floor(random(0)), 13);
        }
        // tile for grass
    ...
    makeCloud(cloudX, random(height));
    makeCloud(cloudX + 100, random(height));
    makeCloud(cloudX + 200, random(height));
